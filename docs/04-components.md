...
- **`torch.compile` è opt-in.** `OUROBOROS_QWEN_COMPILE=1` compila il transformer (`OUROBOROS_QWEN_COMPILE_MODE` per scegliere la modalità); di default è spento. È un costo fisso pagato al primo batch e ammortizzato su tutti i successivi, quindi conviene solo con il target residente per l'intero run — con l'unload aggressivo ricompilerebbe a ogni batch. Viene saltato automaticamente sul percorso con offload, dove gli hook di accelerate spostano i pesi a metà grafo e producono ricompilazioni invece che accelerazioni. Gli errori di Dynamo sono soppressi: una leva di velocità deve degradare a esecuzione eager, non trasformare un run da giorni in un batch di `outcome="error"`.
