...
- Default target is local FLUX.2-klein-4B via mflux (~5 GB). Three backends are wired (`target_backend: Literal["flux", "diffusers", "qwen-image"]` in `src/config.py`, selectable via `--target-backend`): `flux` (FLUX.2-klein-4B via mflux, Apple Silicon, the default for the 16 GB Mac), `diffusers` (the **same** FLUX.2-klein-4B via HuggingFace diffusers on **NVIDIA CUDA** — cloud GPU boxes like RunPod, needs the `[diffusers]` extra) and `qwen-image` (Qwen-Image 20B, NVIDIA CUDA, same extra — the only backend that is a different *model*, not just a different platform). The factory `build_target()` in [src/targets/base.py](src/targets/base.py) keeps adding a further backend (DALL-E, Imagen, SDXL on Vertex) a one-file change without touching the loop.
- **Target sampling params are per-backend, not global.** `--target-steps` / `--target-size` / `--target-quantize` default to `None` and are resolved against `TARGET_DEFAULTS` by `config.resolve_target_params()` (klein is guidance-distilled: 4 steps / 512 px; Qwen-Image is not: 50 steps / 1024 px / `true_cfg_scale=4.0`). `RunConfig` stores **already-resolved** values so `meta.json` records what actually ran. The `--flux-*` spellings survive as argparse aliases on the same `dest`. Any new backend needs a `TARGET_DEFAULTS` row, or it silently inherits klein's 4 steps.
- `qwen-image` wants `--no-aggressive-unload`: the default unload drops the target after every batch, and re-quantizing a 20B pipeline per iteration costs minutes. The CLI warns at startup; `enable_model_cpu_offload()` keeps idle VRAM low anyway.
...
| `targets/base.py` | `TargetBackend` Protocol, `SampleResult`, `RateLimiter`, `build_target()` factory. Dispatches `"flux"` / `"diffusers"` / `"qwen-image"`; raises ValueError for any other name |
...
| `targets/diffusers_flux.py` | `FluxDiffusersTarget` — FLUX.2-klein-4B via HuggingFace diffusers on NVIDIA CUDA (RunPod/Lambda/Colab). Drop-in for `FluxLocalTarget` where mflux is unavailable; `quantize_bits` 4→NF4 (~3.5 GB VRAM) / 8 (~6.5 GB) / else bf16 (~11 GB). Needs `[diffusers]` extra |
| `targets/qwen_image.py` | `QwenImageTarget` — Qwen-Image 20B via diffusers on NVIDIA CUDA. Quantizes transformer **and** text encoder (Qwen2.5-VL-7B is ~15 GB in bf16 alone) via `PipelineQuantizationConfig`; 4→NF4 (~18 GB VRAM) / 8 (~30 GB) / else bf16 (~60 GB). Defaults 50 steps / 1024 px / `true_cfg_scale=4.0`. Needs `[diffusers]` extra |
...
- Every target backend defers its heavy imports (mflux, diffusers, torch) to `_load()`, so *constructing* any target is safe where the dependency is missing. That is what lets `--dry-run` validate a CUDA config on the Mac and `tests/test_targets_factory.py` cover all three backends with no GPU. Don't hoist those imports to module level, and don't re-export the CUDA backends from `targets/__init__.py`.
